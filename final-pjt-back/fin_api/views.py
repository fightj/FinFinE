...
def get_popular_products(request):
    # 예금 상품 중 인기 상품 3개
    popular_deposits = DepositProduct.objects.annotate(
        interest_count=Count('interest_user')
    ).order_by('-interest_count')[:4]

    # 적금 상품 중 인기 상품 3개
    popular_savings = SavingProduct.objects.annotate(
        interest_count=Count('interest_user')
    ).order_by('-interest_count')[:4]

    # 예금 상품에 연결된 옵션 정보 가져오기
    deposit_data = [
        {
            'id': deposit.id,
            'name': deposit.fin_prdt_nm,
            'company': deposit.kor_co_nm,
            ...
            'options': [
                ...
                } for option in deposit.depositoption_set.all()  # Related name 기본값 활용
            ]
        } for deposit in popular_deposits
    ]

    # 적금 상품에 연결된 옵션 정보 가져오기
    saving_data = [
        {
            'id': saving.id,
            ...
            'options': [
                {
                    ...
                    'intr_rate': option.intr_rate,
                    'intr_rate2': option.intr_rate2
                } for option in saving.savingoption_set.all()  # Related name 기본값 활용
            ]
        } for saving in popular_savings
    ]

    return JsonResponse({'deposits': deposit_data, 'savings': saving_data})