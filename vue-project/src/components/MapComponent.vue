<template>
  <div class="map-component-under">
    ...
    <!-- 지도를 표시할 컨테이너 -->
    <div id="mapContainer" style="width: 50%; height: 350px"></div>
  </div>
</template>
...
const loadKakaoMap = function () {
  ...
  const KAKAO_KEY = '74a3f6a524d9744140a08ae7908fbcd3'
...
@use 'sass:map'; // sass:map 모듈을 가져옵니다.
...
.map-search-btn {
  width: fit-content;
  background-color: map.get($colors, 'second'); // map.get() 사용
  color: map.get($colors, 'fifth'); // map.get() 사용
}
</style>
